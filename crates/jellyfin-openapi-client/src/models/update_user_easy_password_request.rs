/*
 * Jellyfin API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 10.8.3
 * 
 * Generated by: https://openapi-generator.tech
 */

/// UpdateUserEasyPasswordRequest : The update user easy password request body.



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct UpdateUserEasyPasswordRequest {
    /// Gets or sets the new sha1-hashed password.
    #[serde(rename = "NewPassword", skip_serializing_if = "Option::is_none")]
    pub new_password: Option<String>,
    /// Gets or sets the new password.
    #[serde(rename = "NewPw", skip_serializing_if = "Option::is_none")]
    pub new_pw: Option<String>,
    /// Gets or sets a value indicating whether to reset the password.
    #[serde(rename = "ResetPassword", skip_serializing_if = "Option::is_none")]
    pub reset_password: Option<bool>,
}

impl UpdateUserEasyPasswordRequest {
    /// The update user easy password request body.
    pub fn new() -> UpdateUserEasyPasswordRequest {
        UpdateUserEasyPasswordRequest {
            new_password: None,
            new_pw: None,
            reset_password: None,
        }
    }
}


